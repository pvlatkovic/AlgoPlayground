...
namespace org.pv.AlgoPlayground.Graphs.BuildOrder
{
	public class Solution2
	{
		...
		*/
		...
		public static int[] GetBuildOrder(int[] projects, int[,] dependencies)
		{
			...
			for	(int i=0; i < projects.Length; i++)
			{
				...
				if(!visited[i])
				{
					...
					dfs(project, graph, visited, visitedNodes);
		...
		private static void dfs(int nodeIndex, Dictionary<int, List<int>> graph, bool[] visited, List<int> visitedNodes)
		{
			...
			if(graph.ContainsKey(node))
			{
				...
				foreach(var edge in edges)
				{
					if(!visited[edge])
					{
						dfs(edge, graph, visited, visitedNodes);
					}
				}
			}
